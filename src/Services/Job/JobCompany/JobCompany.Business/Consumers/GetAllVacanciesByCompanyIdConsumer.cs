...
{
    public class GetAllVacanciesByCompanyIdConsumer(JobCompanyDbContext _context)
        : IConsumer<GetAllVacanciesByCompanyIdDataRequest>
    {
        public async Task Consume(ConsumeContext<GetAllVacanciesByCompanyIdDataRequest> context)
        {
            var vacancies = await _context
                .Vacancies.Where(x => x.CompanyId == context.Message.CompanyId)
                .Include(x => x.Company)
                .ToListAsync();
            ...
            if (vacancies is null)
            {
                await context.RespondAsync(
                    new GetAllVacanciesByCompanyIdDataResponse { Vacancies = [] }
                );
                return;
            }
